...
export async function onRequestGet({ request, env }) {
  ...
  const { results } = await env.DB.prepare(
    'SELECT id, start_date, start_onah, hefsek_date, mikveh_date, notes, clean_checks, created_at, updated_at FROM cycles WHERE user_id = ? ORDER BY start_date DESC'
  ).bind(ownerId).all();
  ...
  return jsonOk({
    cycles: results.map(r => ({
      ...
      notes: r.notes,
      cleanChecks: r.clean_checks ? JSON.parse(r.clean_checks) : {},
